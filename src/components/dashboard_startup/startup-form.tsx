...
const AREA_ATUACAO_SUGESTOES: Array<{ area: string; keywords: string[] }> = [
  {
    area: 'Fintech',
    keywords: ['fintech', 'finance', 'pagamento', 'bank', 'credito', 'crédito'],
  },
  {
    area: 'E-commerce',
    keywords: ['comércio', 'commerce', 'loja', 'marketplace', 'varejo online'],
  },
  {
    area: 'SaaS',
    keywords: ['software', 'plataforma', 'sas', 'gestão', 'service'],
  },
  {
    area: 'EdTech',
    keywords: ['educ', 'ensino', 'aprend', 'treinamento', 'curso'],
  },
  {
    area: 'HealthTech',
    keywords: ['saúde', 'health', 'clínic', 'medic', 'hospital'],
  },
  { area: 'AgTech', keywords: ['agro', 'agrícola', 'campo', 'fazenda'] },
  {
    area: 'Marketplace',
    keywords: ['marketplace', 'hub de ofertas', 'plataforma de oferta'],
  },
  {
    area: 'Logística',
    keywords: ['logístic', 'transporte', 'entrega', 'last mile', 'supply'],
  },
  {
    area: 'Energia',
    keywords: ['energia', 'solar', 'power', 'renovável', 'elétrica'],
  },
...
const startupSchema = z.object({
  ...
  meta_captacao: z
    ...
    .refine((val) => val === 0 || (val >= 100000.0 && val <= 15000000.0), {
      message:
        'Meta de captação deve ser 0 ou entre R$ 100.000,00 e R$ 15.000.000,00',
    }),
  equity_oferecido: z
    .number()
    .min(0.1)
    .max(100, 'Equity deve estar entre 0.1% e 100%'),
  descricao_objetivo: z
    .string()
    .min(20, 'Objetivo deve ter pelo menos 20 caracteres'),
});
  ...
  const handleConsultCnpj = async () => {
    ...
    try {
      ...
      if (dataFundacao) {
        form.setValue(
          'data_fundacao',
          new Date(dataFundacao).toISOString().split('T')[0],
          {
            shouldDirty: true,
            shouldTouch: true,
            shouldValidate: true,
      ...
      if (!initialData) {
        ...
        if (!Number.isNaN(receitaPresumida) && receitaPresumida > 0) {
          const metaSugerida = Math.min(
            Math.max(receitaPresumida * 0.1, 100000),
            15000000,
          );
          ...
          <div className="flex flex-col justify-end gap-4 pt-6 sm:flex-row">
            ...
            <div className="flex items-end">
              <Button
                type="submit"
                disabled={isLoading}
                className="h-10 w-full bg-[#d500f9] px-8 font-medium text-white hover:bg-[#d500f9]/90 sm:w-auto"
              >
                {isLoading ? (
